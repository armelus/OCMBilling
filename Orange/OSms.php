...
class OSms
{
    ...
    )
    {
        ...
        if ($httpCode !== $successCode) {
            ...
            if (!empty($response['error_description'])) {
                $errorMessage = 'error_description: ' . $response['error_description'];
            } elseif (!empty($response['error'])) {
                $errorMessage = 'error: ' . $response['error'];
            } elseif (!empty($response['description'])) {
                $errorMessage = 'description: ' . $response['description'];
            } elseif (!empty($response['message'])) {
                $errorMessage = 'message: ' . $response['message'];
            } elseif (!empty($response['requestError']['serviceException'])) {
                $errorMessage = 'requestError - serviceException: ' . $response['requestError']['serviceException']['text']
                    . ' ' . $response['requestError']['serviceException']['variables'];
            } elseif (!empty($response['requestError']['policyException'])) {
                $errorMessage = 'requestError - policyException' . $response['requestError']['policyException']['text']
    ...
    )
    {
        ...
        $headers = array(
            ...
            'X-Orange-ISE2:' . str_replace('acr:', '', $receiverAddress),
            'X-Orange-MCO: OCM'
        );
